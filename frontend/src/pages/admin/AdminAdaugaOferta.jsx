        ...
        <div className={`grid grid-cols-2 px-3 gap-3 pb-3 border-b-button/30 mt-3 border-b ${visibleBara?"max-modf:grid-cols-1":" max-modf1:grid-cols-1"}`}>
              ...
              <ul className={`absolute transition-all duration-500 ease-in-out overflow-y-scroll scrollbar-thin overflow-hidden ${visibleHotelList?"max-h-40 opacity-100":"max-h-0 opacity-0"} bg-background z-30  w-full`}>
                {
                  hotels.map((hotel,iHotel)=>(
                    <li className={`border border-button/30 px-3 py-3 hover:bg-button/60 cursor-pointer transition-all duration-300 ease-in-out flex items-center space-x-3`} key={iHotel}>
                      <div className='w-30 h-20 shrink-0'>
                        <img className='w-full h-full' src={hotel.img} alt="" />
                      </div>
              ...
              <ul className={`absolute transition-all duration-500 ease-in-out overflow-y-scroll scrollbar-thin overflow-hidden ${visibleCameraList?"max-h-40 opacity-100":"max-h-0 opacity-0"} bg-background z-30  w-full`}>
                {
                  hotels[0].rooms.map((camera,iCamera)=>(
                    <li className={`border border-button/30 px-3 py-3 hover:bg-button/60 cursor-pointer transition-all duration-300 ease-in-out flex items-center space-x-3`} key={iCamera}>
                      <div className='w-30 h-20 shrink-0'>
                        <img className='w-full h-full' src={camera.image} alt="" />
                      </div>
                      <div className='text-gray-400'>
                        <p>Cod: {camera.codCamer}</p>
                        <p>{camera.title}</p>
          ...
          <div className=''>
            ...
            <div className='relative mt-3'>
              <input type="number" className='border border-button/30  pl-3 py-1.5 rounded-sm outline-0  w-full' maxLength={3} placeholder='Ex: 20' min={0} max={100} name="" id="" />
              <p className='absolute text-gray-400 right-6 top-1/2 -translate-y-1/2'>procent (%)</p>
            </div>
          </div>
