...
const AdminOferte = () => {
  ...
  let nrTotalOferte = 0;
  let nrTotalOferteActive = 0;
  let nrTotalOferteInactive = 0;
  let nrTotalOferteProgramate = 0;

  let listCoduri = [];
  hotels.forEach(hotel=>
    hotel.rooms.forEach(room=>{
      if(room.oferta.cod !=="") {
        listCoduri.push(room.oferta.cod);
      }
    })
    
  )
  const ultimulCod = listCoduri[listCoduri.length-1];
  const penultimulCod = listCoduri[listCoduri.length-2];
  console.log(penultimulCod)
  hotels.forEach(hotel=>{
    const nrOfertaCamera=hotel?.rooms?.filter(room=>
    (
      room?.oferta?.stare==="Activ" ||
      room?.oferta?.stare === "Programat" ||
      room?.oferta?.stare === "Inactiv" 
    )
    ).length || 0;
    nrTotalOferte += nrOfertaCamera;
    }
  );

  hotels?.forEach(hotel=>{
    const nrActive = hotel?.rooms?.filter(room =>room?.oferta?.stare === "Activ").length || 0;
    nrTotalOferteActive += nrActive;
  });
  hotels?.forEach(hotel=>{
    const nrProgramate = hotel?.rooms?.filter(room=>room?.oferta?.stare === "Programat").length || 0;
    nrTotalOferteProgramate += nrProgramate;
  });
  hotels?.forEach(hotel=>{
    const nrInactive = hotel?.rooms?.filter(room=>room?.oferta?.stare === "Inactiv").length || 0;
    nrTotalOferteInactive +=nrInactive;
  })
      ...
      {visibleAdminOfertaPage &&(
        <div >
          ...
          <ul className={`text-[14px] mt-4 grid grid-cols-4 gap-3 ${visibleBara?"max-modf:grid-cols-1":"max-modf:grid-cols-2 max-modf2:grid-cols-1"}`}>
            {
              [
                {icon:"nrTot",textIc:"text-button",bgIc:"bg-button/30",nume:"Total oferte",nr:nrTotalOferte,nrText: "+3", text:"fata de luna trecuta",},
                {icon:"activ",textIc:"text-green-400",bgIc:"bg-green-400/30",nume:"Active",nr:nrTotalOferteActive,nrText: "+3", text:"fata de luna trecuta",},
                {icon:"programat",textIc:"text-orange-400",bgIc:"bg-orange-400/30",nume:"Programate",nr:nrTotalOferteProgramate,nrText: "-1", text:"fata de luna trecuta",},
                {icon:"inactive",textIc:"text-red-400",bgIc:"bg-red-400/30",nume:"Inactive",nr:nrTotalOferteInactive,nrText: "", text:"Nicio schimbare",},
              ].map((par,i)=>
            ...
            <div className={`w-full modf7:flex modf7:justify-center`}>
              <div className='relative modf7:w-[80%] '>
                ...
                <ul className={`absolute ${visibleHot? "max-h-40 opacity-100": "max-h-0 opacity-0"} w-full bg-background z-100 overflow-hidden overflow-y-scroll scrollbar-none transition-all duration-500 ease-in-out`}>
                  ...
                  {
                    hotels?.map((hotel,i)=>(
                      <li key={i} className={`px-3 py-1 border cursor-pointer border-button/30 hover:bg-button/60 transition-all duration-300 ease-in-out`}><p>{hotel.nume}</p></li>
              ...
              <ul  className='mt-4 text-gray-400'>
                {
                  ...
                  hotel?.rooms?.filter((room)=>
                    ...
                    <li key={room.oferta.cod} className='border-y border-y-button/30 px-3 py-3 min-w-max '>
                      <div className='flex space-x-3 items-center'>
                        <div className={`${visibleBara? "max-modf7:w-55": "max-modf7:w-63"} modf7:flex-2  shrink-0 transition-[width] duration-300 ease-in-out flex items-center space-x-3`}>
                          <div className='w-15 h-15 shrink-0'>
                            <img className='w-full h-full object-center rounded-lg' src={room.image} alt="" />
                          </div>
                          <div >
                            <p className='text-white'>{room.title},  </p>
                            <p className='text-white'>{room?.oferta?.reducere} reducere  </p>
                            <p>Cod: {room?.oferta?.cod}</p>
                          </div>
                        </div>
                        ...
                        <div className={`${visibleBara? "max-modf7:w-30": "max-modf7:w-38"} modf7:flex-1  shrink-0 transition-[width] duration-300 ease-in-out`}>
                          <p className='text-white'>{room?.oferta?.reducere}</p>
                        ...
                        <div className={`${visibleBara? "max-modf7:w-23": "max-modf7:w-30"} modf7:flex-1  shrink-0 transition-[width] duration-300 ease-in-out`}>
                          <p className={`${room?.oferta?.bgStare} ${room?.oferta?.textStareCol} bg-green-400/30 text-center rounded-sm`}>{room?.oferta?.stare}</p>
                        </div>
                        <div className={`${visibleBara? "max-modf7:w-28": "max-modf7:w-33"} modf7:flex-1  shrink-0 transition-[width] duration-300 ease-in-out`}><p>{room?.venitTotal} RON</p></div>
                        <div className={`${visibleBara? "max-modf7:w-18": "max-modf7:w-21"} modf7:flex-1  text-center shrink-0 transition-[width] duration-300 ease-in-out`}>
                          <div className='flex justify-center space-x-3'>
                            <div className='relative'>
                              <button onClick={()=>{setVisibleEditAct(visibleEditAct === room.oferta.cod? null: room.oferta.cod); document.body.classList.add("overflow-hidden"); setVisibleAct(null)}} className='px-1.5 py-1.5 cursor-pointer border rounded-sm border-button/30'>
                                      ...
                                      <div className=' modf8:space-x-3 mt-1.5 flex max-modf8:flex-col w-full'>
                                        <div className=''>
                                          <div className='w-35 h-32 max-modf8:w-full shrink-0 object-center max-modf8:h-45'>
                                            <img className='w-full h-full rounded-lg' src={room.image} alt="" />
                                          </div>
                                          <div>
                                        ...
                                        <div className='w-full  space-y-1'>
                                          ...
                                          <div className='space-y-0.5'>
                                            ...
                                            <input className='border w-full border-button/30 rounded-sm px-3 py-1 placeholder:text-white text-white outline-0' type="text" name="" id="" defaultValue={`${room.title}`}/>
                                          </div>

                                          <div className='space-y-0.5'>
                                            <p className='text-start'>Nume hotel</p>
                                            <input className='border w-full border-button/30 rounded-sm px-3 py-1 placeholder:text-white text-white outline-0' type="text" name="" id="" defaultValue={`${hotel.nume}`}/>
                                          </div>

                                          <div className='grid grid-cols-2 gap-1.5 mt-1.5'>
                                            <div className='space-y-0.5'>
                                              <p className='text-start'>Cod oferta</p>
                                              <input className='border w-full border-button/30 rounded-sm px-3 py-1 placeholder:text-white text-white outline-0' type="text" name="" id="" defaultValue={`${room.oferta.cod}`}/>
                                            </div>

                                            <div className='space-y-0.5'>
                                              <p className='text-start'>Reducere</p>
                                              <div>
                                                <input className='border w-full border-button/30 rounded-sm px-3 py-1 placeholder:text-white text-white outline-0' type="text" name="" id="" defaultValue={`${room.oferta.reducere}`}/>
                                              </div>
                                            </div>
                                          ...
                                          <div className='space-y-0.5'>
                                            ...
                                            <div className='relative'>
                                              ...
                                              <ul className={`absolute  z-100 bg-background w-full text-start  overflow-y-scroll overflow-hidden scrollbar-none ${visibleEditStatus?"max-h-40 opacity-100": "max-h-0 opacity-0"} transition-all duration-500 ease-in-out`}>
                                                {
                                                  ["Activ","Inactiv","Programat"].map((stareEd,iEd)=>(
                                          ...
                                          <div className='space-y-0.5 text-start'>
                                            ...
                                            <div className='relative'>
                                              <textarea className='w-full outline-0 px-3 border border-button/30 h-27 scrollbar-thin resize-none' name="" defaultValue={room.descriereCamera} onChange={(e)=>setTextEd(e.target.value)} maxLength={maxLit} id=""></textarea>
                            ...
                            <div className='relative'>
                              <button onClick={()=>setVisibleAct(visibleAct===room.oferta.cod? null: room.oferta.cod)} className='px-1.5 py-1.5 border cursor-pointer rounded-sm border-button/30'>
                                <BsThreeDotsVertical />
                              </button>
                              ...
                              {visibleAct === room.oferta.cod && (
                                <div className={`absolute -right-1  ${(room.oferta.cod === ultimulCod || room.oferta.cod === penultimulCod)?"bottom-9":"top-9"}`}>
                                  <div  className='border border-button px-1.5 py-1 rounded-lg bg-background z-100 text-white relative'>
                                        ...
                                        listaAct.map((par,iList)=>{
                                          ...
                                          return (
                                            <li key={iList} className='relative'>
                                              <button onClick={()=>{setModal({actiune:iList,hotel,room}); document.body.classList.add("overflow-hidden"); setVisibleAct(null)}} className={`flex items-center space-x-3 ${iList===listaAct.length-1? "hover:bg-red-400/30 text-red-400 border-t border-t-button/30":"hover:bg-button/30"} transition-all duration-300 ease-in-out cursor-pointer py-1 px-1 w-full`}>
                                                <div>
                                                  {IconLiAct && <IconLiAct/>}
                                                </div>
                                                <p>{par.nume}</p>
                                    ...
                                    <div className={`absolute ${(room.oferta.cod === ultimulCod || room.oferta.cod === penultimulCod) ? "-bottom-2": "-top-2"} right-3  w-2 h-2 bg-button`} style={{clipPath: (room.oferta.cod === ultimulCod || room.oferta.cod === penultimulCod) ? "polygon(0 0, 100% 0, 50% 100%)":"polygon(0 100%, 100% 100%, 50% 0)"}}></div>
                                    <div className={`absolute ${(room.oferta.cod === ultimulCod || room.oferta.cod === penultimulCod) ? "-bottom-1.75": "-top-1.75"} right-3  w-2 h-2 bg-background`} style={{clipPath: (room.oferta.cod === ultimulCod || room.oferta.cod === penultimulCod) ? "polygon(0 0, 100% 0, 50% 100%)":"polygon(0 100%, 100% 100%, 50% 0)"}}></div>
                                  </div>
                                </div>
                              )
                              }
                        ...
                        <div className='w-165 max-h-[95vh] overflow-y-auto overflow-hidden scrollbar-thin border border-button bg-background rounded-lg py-3 px-3'>
                          ...
                          <div className='mt-1 flex modf8:space-x-3 max-modf8:flex-col'>
                            <div className='flex-2 '>
                              <div className='flex max-modf8:flex-col modf8:space-x-3'>
                                <div className='w-35 shrink-0 mt-1.5 h-30 max-modf8:w-full max-modf8:h-50'>
                                  <img className='w-full rounded-lg h-full' src={modal.room.image} alt="" />
                                </div>
                                <div className='w-full max-modf8:mt-1.5'>
                                  <p className='text-white text-[19px]'>{modal.room.title}</p>
                                  <p>Cod oferta</p>
                                  <p className='text-white text-[19px]'>{modal.room.oferta.cod}</p>
                                  <p>Hotel</p>
                                  <p className='text-white'>{modal.hotel.nume}</p>
                                  <p>{modal.hotel.locatie}</p>
                                </div>
                              </div>
                            ...
                            <div className='flex-1 border border-button/30 px-1.5 py-1 rounded-lg max-modf8:mt-1.5'>
                              <p>Reducere</p>
                              <p className='text-green-400 text-[19px]'>{modal.room.oferta.reducere}</p>
                              <p>din pretul camerei</p>
                              ...
                              <div className='mt-1.5 space-y-0.5'>
                                <p>Creata la data:</p>
                                <p>15 mai 2026, 10:30</p>
                              </div>
                              <div className='mt-1.5 space-y-0.5'>
                                <p>Ultima modificare:</p>
                                <p>28 mai 2026, 14:22</p>
                              </div>
                            </div>
...

                      {
                        modal.actiune === 1 && (
                          <div className='border border-button px-3 py-3 overflow-y-auto overflow-hidden scrollbar-thin max-h-[95vh] rounded-lg w-125 bg-background'>
                            ...
                            <div className='mt-1 '>
                              <p>Creeaza o copie al ofertei pentru camera {modal.room.title} din hotelul {modal.hotel.nume}.</p>
                            ...
                            <div className='border border-button/30 px-3 py-3 rounded-lg mt-2 grid grid-cols-2 max-modf8:grid-cols-1 gap-3'>
                              <div>
                                <p>Nume hotel: <span className='text-white ml-3'>{modal.hotel.nume}</span></p>
                                <p>Nume camera: <span className='text-white ml-3'>{modal.room.title}</span></p>
                                <p>Locatie: <span className='text-white ml-3'>{modal.hotel.locatie}</span></p>
                              </div>

                              <div>
                                <p>Oferta de: <span className='text-white ml-3'>{modal.room.oferta.reducere}</span></p>
                                <p>Stare: <span className='text-white ml-3'>{modal.room.oferta.stare}</span></p>
                                <p>Perioada: <span className='text-white ml-3'>01 iun. 2026-30 iun. 2026</span></p>
                              </div>
                            </div>
...

                      {
                        modal.actiune === 2 && (
                          <div className='border border-button px-3 py-3 overflow-y-auto overflow-hidden scrollbar-thin max-h-[95vh] rounded-lg w-110 bg-background'>
                            ...
                            {
                              modal.room.oferta.stareActivareOferta === false && (
                            ...
                            {
                              modal.room.oferta.stareActivareOferta === true && (
                                <div className='text-center'>
                            ...
                            <div className='mt-2'>
                              ...
                              <ul className='list-disc marker:text-button mt-1 border border-button/30 px-3 py-3 rounded-lg'>
                                {
                                  modal.room.oferta.stareActivareOferta === false &&( ["Oferta va fi trecuta ca inactiva","Tarifele vor reveni la valoarea lor initiala","Datele si rezervariile existente vor fi pastrate"].map((dezact,iDezact)=>(
                                ...
                                {
                                  modal.room.oferta.stareActivareOferta === true &&( ["Oferta va fi trecuta ca activa","Tarifele vor reveni la valoarea redusa","Datele si rezervariile existente vor fi pastrate"].map((dezact,iDezact)=>(
                                    <li key={iDezact} className='ml-3'>
                                      <p>{dezact}</p>
                                    </li>
                                  )))
                                  ...
                                  <button onClick={()=>{setModal(null); document.body.classList.remove("overflow-hidden");
                                    setHotels(prev=>prev.map(hotel=>({
                                      ...
                                      rooms: hotel.rooms.map(room=>({
                                          ...room, 
                                          oferta: room.oferta.cod === modal.room.oferta.cod ? {
                                            ...room.oferta,
                                ...
                                modal.room.oferta.stareActivareOferta=== true &&(
                                  ...
                                  setHotels(prev=>prev.map(hotel=>({
                                    ...
                                    rooms: hotel.rooms.map(room=>({
                                      ...room,
                                      oferta: room.oferta.cod === modal.room.oferta.cod ? {
                                        ...room.oferta, stareActivareOferta: false
                                      }: room.oferta
                                    }))
                                  })))  
