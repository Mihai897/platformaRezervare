...
const AdminCamere = () => {
  ...
  let totalCamereDisponibile = 0;
  let totalCamereOcupate = 0;
  let totalCamereAsteptare = 0;
  ...
  hotels.forEach(hotel=>{
    totalCamere += hotel?.rooms?.length || 0 
  })

  hotels.forEach(hotel => {
    const camereDisponibile = hotel?.rooms?.filter(room => room?.disponibil === "Disponibil").length || 0 ;
    totalCamereDisponibile += camereDisponibile;
  });

  hotels?.forEach(hotel=>{
    const camereOcupate = hotel?.rooms?.filter(room => room?.disponibil === "Ocupat").length || 0;
    totalCamereOcupate += camereOcupate;
  })


  hotels?.forEach(hotel=>{
    const nrAsteptare= hotel?.rooms?.filter(room=>room?.disponibil ==="Asteptare").length ||0;
    totalCamereAsteptare += nrAsteptare;
  })
    ...
    <div className='mt-18 '>
      {visibleAdaugaCamera &&(<div className=''>
        ...
        <ul className={`max-modf6:grid-cols-3 max-modf:grid-cols-2 ${visibleBara? "max-modf2:grid-cols-1": "max-modf2:grid-cols-2"} gap-3 mt-4 grid-cols-5 grid `}>
          {
            [
              {icon:"nrHotel",nume:"Total camere", text:"in toate proprietatile", procent:"",bgI:"bg-button/30",textI:"text-button",nr:totalCamere},
              {icon:"disponnibil",nume:"Disponibile", text:"din total", procent:"75%",bgI:"bg-green-400/30",textI:"text-green-400",nr:totalCamereDisponibile},
              {icon:"ocupate",nume:"Ocupate", text:"din total", procent:"+21%",bgI:"bg-red-400/30",textI:"text-red-400",nr:totalCamereOcupate},
              {icon:"asteptare",nume:"In asteptare", text:"din total", procent:"+4%",bgI:"bg-orange-400/30",textI:"text-orange-400",nr:totalCamereAsteptare},
              {icon:"venit",nume:"Venituri din camere", text:"fata de luna trecuta", procent:"+22%",bgI:"bg-green-400/30",textI:"text-green-400",nr:totalVenituriCamere,valuta:"RON"},
            ].map((par,i)=>
            {
        ...
        <div className='mt-4  border border-button/30 text-[14px] py-3 rounded-lg'>
          ...
          <div className='overflow-x-auto min-w-0 scrollbar-thin pb-2'>
            ...
            <ul className='mt-3'>
              ...
              {
                hotels?.map((hotel,i)=>
                hotel?.rooms?.map((room,i1)=>(
                  <li key={room.codCamer} className='border-y border-y-button/30 py-3 space-y-3 min-w-max'>
                    ...
                    <div className='flex space-x-3 px-3 items-center text-gray-400'>
                      ...
                      <div className={`${visibleBara?"max-modf7:w-55":"max-modf7:w-67"} modf7:flex-3  shrink-0 transition-[width] duration-300 ease-in-out flex items-center space-x-3`}>
                        <div className='w-16 rounded-lg h-16 overflow-hidden shrink-0'>
                          <img className='w-full h-full hover:scale-110 transition-all duration-300 ease-in-out rounded-lg' src={room.image} alt="" />
                        </div>
                        <div>
                      ...
                      <div className={`${visibleBara? "max-modf7:w-43":"max-modf7:w-60"} modf7:flex-2 shrink-0 transition-[width] duration-300 ease-in-out`}>
                        <p className='text-white'>{hotel.nume}</p>
                        <p>{hotel.locatie}</p>
                      </div>
                      ...
                      <div className={`${visibleBara? "max-modf7:w-19": "max-modf7:w-23"} modf7:flex-1 shrink-0 transition-[width]  duration-300 ease-in-out`}>
                        <div className='flex justify-center space-x-3  '>
                          <button onClick={()=>{setModalCamAct(()=>({actiune:2,hotel,room})); document.body.classList.add("overflow-hidden"); setVisibleCameraActiune(null)}} className='px-1.5 py-1.5 cursor-pointer border rounded-sm border-button/30'>
                            <GoPencil className='text-button' />
                          </button>
                          <div className='relative'>
                            <button onClick={()=>{setVisibleCameraActiune(visibleCameraActiune === room.codCamer?null:room.codCamer); }} className='px-1.5 py-1.5 border cursor-pointer rounded-sm border-button/30'>
                              <BsThreeDotsVertical />
                            </button>
                            {
                            visibleCameraActiune ===room.codCamer && (
                              <div className={`absolute  mt-3 right-10 ${visibleCameraActiune === camereleCodLista[camereleCodLista.length-1]? "-bottom-3":"top-1/2 -translate-y-1/2"}  z-600  `}>
                                <div className='bg-background w-47 relative border rounded-lg border-button py-1.5'>
                                  <ul className=' px-1'>
                                    {
                                      camAct.map((act,iAct)=>{
                                        ...
                                        return(
                                        <li onClick={()=>{setModalCamAct(()=>({actiune:iAct,hotel,room})); document.body.classList.add("overflow-hidden"); setVisibleCameraActiune(null)}} key={iAct} className={` px-3 cursor-pointer py-1.5 transition-all duration-300 ease-in-out flex items-center space-x-3 ${iAct === camAct.length-1? "text-red-400 hover:bg-red-400/30":"hover:bg-button/60 text-white"}`}>
                                  ...
                                  <div className={`absolute w-3 h-3 bg-button  -right-3 ${visibleCameraActiune === camereleCodLista[camereleCodLista.length-1]? "bottom-4.5":"top-1/2 -translate-y-1/2 -mt-3"}`} style={{clipPath: "polygon(0% 100%, 100% 50%, 0 0"}}></div>
                                  <div className={`absolute w-3 h-3 bg-background  -right-2.75 ${visibleCameraActiune === camereleCodLista[camereleCodLista.length-1]? "bottom-4.5":"top-1/2 -translate-y-1/2 -mt-3"}`} style={{clipPath: "polygon(0% 100%, 100% 50%, 0 0"}}></div>
                                </div>
                              </div>
                              )
                            }
                          ...
                          <div className='border border-button bg-background overflow-y-auto overflow-hidden max-h-[95vh] scrollbar-thin px-3 py-3 w-165 rounded-lg'>
                            ...
                            <div className='grid grid-cols-2 max-modf2:grid-cols-1 mt-2 gap-3'>
                              <div className=''>
                                <div className='w-full h-45'>
                                  <img className='w-full h-full' src={modalCamAct.room.image} alt="" />
                                </div>

                                <div className='flex space-x-1 items-center mt-1.5'>
                                  <div className='grid grid-cols-4 gap-1.5 flex-1'>
                                    <img className='w-full h-15' src={modalCamAct.room.image} alt="" />
                                    <img className='w-full h-15' src={modalCamAct.room.image} alt="" />
                                    <img className='w-full h-15' src={modalCamAct.room.image} alt="" />
                                    <img className='w-full h-15' src={modalCamAct.room.image} alt="" />
                                  </div>
                              ...
                              <div className='space-y-1'>
                                ...
                                <div className='grid grid-cols-2 gap-3'>
                                  <p>Cod camer</p>
                                  <p className='text-gray-400'>{modalCamAct.room.codCamer}</p>
                                </div>
                                <div className='grid grid-cols-2 gap-3'>
                                  <p>Hotel</p>
                                  <p className='text-gray-400'>{modalCamAct.hotel.nume}</p>
                                </div>
                                <div className='grid grid-cols-2 gap-3'>
                                  <p>Nume camera</p>
                                  <p className='text-gray-400'>{modalCamAct.room.title}</p>
                                ...
                                <div className='grid grid-cols-2 gap-3'>
                                  <p>Tip vedere</p>
                                  <p className='text-gray-400'>{modalCamAct.room.tipVedere}</p>
                                </div>
                                <div className='grid grid-cols-2 gap-3'>
                                  <p>Descriere</p>
                                  <p className='text-gray-400'>{modalCamAct.room.descriereCamera}</p>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className='mt-2 '>
                              ...
                              <div className='border border-button/30 px-3 py-1.5 max-modf8:grid-cols-2 max-modf2:gap-1.5 rounded-lg grid grid-cols-4 mt-2'>
                                ...
                                <div className='text-center '>
                                  <p>Capacitate</p>
                                  <p className='text-gray-400'>{modalCamAct.room.capacitate} </p>
                                </div>
                                <div className='text-center '>
                                  <p>Paturi</p>
                                  <p className='text-gray-400'>{modalCamAct.room.tipulPatului} </p>
                                </div>
                                <div className='text-center '>
                                  <p>Pret/noapte</p>
                                  <p className='text-gray-400'>{modalCamAct.room.price} RON</p>
                                </div>
                              </div>
                            </div>
                            <div className='mt-2'>
                              ...
                              <ul className='grid grid-cols-3 gap-x-6 gap-1.5 max-modf8:grid-cols-2 list-disc text-gray-400 marker:text-button ml-5'>
                                {
                                  modalCamAct.room.facilitatiCamera.map((fac,iFaci)=>(
                                    <li key={iFaci}>
                                      <p>{fac}</p>
                                    </li>
                                  ))
                          ...
                          <div className='border border-button bg-background overflow-y-auto overflow-hidden max-h-[95vh] scrollbar-thin px-3 py-3 rounded-lg w-195'>
                            ...
                            <div className='flex justify-between space-x-4 items-center'>
                              <div className='flex modf8:space-x-3 max-modf8:space-y-1.5 modf8:items-center max-modf8:flex-col'>
                                <div className='w-15 h-15 mt-1.5 overflow-hidden border border-button/30 rounded-sm shrink-0'>
                                  <img className='rounded-sm  object-center  hover:scale-110 transition-all duration-300 ease-in-out  w-full h-full shrink-0' src={modalCamAct.room.image} alt="" />
                                </div>

                                <div className='space-y-1.5 max-modf8:space-y-0.5'>
                                  <p>{modalCamAct.room.title}</p>
                                  <p className='text-gray-400'>{modalCamAct.room.codCamer} &middot; {modalCamAct.hotel.nume}, {modalCamAct.hotel.locatie}</p>
                                </div>
                              </div>
                              
                              <div>
                                <p className={`${modalCamAct.room.bgDis} ${modalCamAct.room.culDis} px-3 py-0.5 rounded-sm`}>{modalCamAct.room.disponibil}</p>
                              </div>
                            </div>
                            ...
                            <div className='mt-2 border border-button/30 py-3 rounded-sm min-w-0 overflow-x-auto scrollbar-thin'>
                              <div className='pr-3'>
                                ...
                                <ul>
                                  {
                                    clienti.map(client=>client.rezervari.filter(rezervare=>rezervare.codCamer === modalCamAct.room.codCamer).map((rez,irez)=>(
                                      <li key={modalCamAct.room.codCamer} className='flex space-x-3 py-3 border-y items-center border-y-button/30 min-w-max text-gray-400'>
                                        <div className='shrink-0 flex items-center space-x-3  w-58 px-3'>
                                          <div className='w-10 h-10 rounded-full shrink-0'>
                                            <img className='w-full h-full rounded-full' src={client.pozaProfil} alt="" />
                                          </div>
                                          <div className=''>
                                            <p>{client.nume}</p>
                                            <p className='break-all text-[13px]'>{client.email}</p>
                                          </div>
                                        </div>
                                        <div className='shrink-0  w-23'>{rez.checkIn}</div>
                                        <div className='shrink-0  w-23'>{rez.checkOut}</div>
                                        <div className='shrink-0  w-10 text-center'>{rez.periadaCazare}</div>
                                        <div className='shrink-0  w-20'>{rez.nrOaspetiAdulti} {rez.nrOaspetiCopii}</div>
                                        <div className='shrink-0  w-16'>{rez.pretRez} RON</div>
                                        <div className={`shrink-0  w-20 text-center `}><p className={`${rez.status==="Finalizata"?"text-green-400 bg-green-400/30": "text-red-400 bg-red-400/30"} w-[90%]`}>{rez.status}</p></div>
                                      </li>
                                    )))
                                  }
                                </ul>
                            ...
                            <div className='grid grid-cols-2 max-modf8:grid-cols-1 gap-3 mt-1.5'>
                              <div className=' space-y-1.5'>
                                ...
                                <div className='text-gray-400 space-y-1'>
                                  ...
                                  <input type="text" defaultValue={modalCamAct.room.codCamer} className='border w-full border-button/30 px-3 py-1.5 rounded-lg outline-0 text-white placeholder:text-white' />
                                </div>

                                <div className='text-gray-400 space-y-1'>
                                  <p >Hotel <span className='text-red-600'>*</span></p>
                                  <input type="text" defaultValue={modalCamAct.hotel.nume} className='border w-full border-button/30 px-3 py-1.5 rounded-lg outline-0 text-white placeholder:text-white' />
                                </div>

                                <div className='text-gray-400 space-y-1'>
                                  <p >Descriere </p>
                                  <textarea type="text" defaultValue={modalCamAct.room.descriereCamera} className='border w-full border-button/30 px-3 py-1.5 rounded-lg outline-0 text-white placeholder:text-white resize-none h-24' ></textarea>
                                </div>
                              ...
                              <div className='space-y-1.5'>
                                ...
                                <div className='text-gray-400 space-y-1'>
                                  <p >Capacitate </p>
                                  <div className='relative w-full'>
                                    <input type="text" defaultValue={modalCamAct.room.capacitate} className='border w-full border-button/30 pl-3 pr-20 py-1.5 rounded-lg outline-0 text-white placeholder:text-white' />
                                    <p className='absolute text-white right-3 top-1/2 -translate-y-1/2'>persoane</p>
                                  </div>
                                </div>
                                ...
                                <div className='text-gray-400 space-y-1'>
                                  <p >Paturi </p>
                                  <div className='relative w-full'>
                                    <input type="text" defaultValue={modalCamAct.room.tipulPatului} className='border w-full border-button/30 px-3 py-1.5 rounded-lg outline-0 text-white placeholder:text-white' />
                                  </div>
                                </div>
                                ...
                                <div className='text-gray-400 space-y-1'>
                                  <p >Pret/noapte </p>
                                  <div className='relative w-full'>
                                    <input type="text" defaultValue={modalCamAct.room.price} className='border w-full border-button/30 pr-12 pl-3 py-1.5 rounded-lg outline-0 text-white placeholder:text-white' />
                                ...
                                <div className='text-gray-400 space-y-1'>
                                  <p >Status </p>
                                  <div className='relative w-full'>
                                    <button onClick={()=>setVisibleEditStats(!visibleEditStats)} className={`border px-3 py-1.5 flex space-x-3 hover:bg-button/60 transition-all duration-300 ${visibleEditStats?"bg-button/60":"bg-transparent"} ease-in-out cursor-pointer justify-between items-center text-start w-full border-button/30  ${modalCamAct.room.disponibil === "Disponibil"?"text-green-400":modalCamAct.room.disponibil==="Ocupat"?"text-red-400":"text-orange-400"}`}>
                                      <p>{modalCamAct.room.disponibil}</p>
                                ...
                                <div>
                                  <div className='w-full h-40 shrink-0'>
                                    <img className='w-full h-full object-center' src={modalCamAct.room.image} alt="" />
                                  </div>
                        ...
                        modalCamAct.actiune === 3 && (
                          <div className='border border-button bg-background overflow-y-auto overflow-hidden max-h-[95vh] scrollbar-thin px-3 py-3 rounded-lg w-100'>
                            ...
                            <div className='flex justify-center '>
                              <div className={` px-3 py-3 rounded-full ${modalCamAct.room.stareActivare === false?"text-orange-400 bg-orange-400/12": "text-green-400 bg-green-400/12"} shrink-0`}>
                                <AiOutlineExclamationCircle className='shrink-0' size={30}/>
                              </div>
                            </div>
                            
                            {
                              modalCamAct.room.stareActivare === false ? (
                                    ...
                                    setHotels(prev =>
                                      prev.map(hotel => ({
                                        ...
                                        rooms: hotel.rooms.map(room =>
                                          room.codCamer === modalCamAct.room.codCamer
                                            ? { ...room, stareActivare: true }
                                            : room
                                        )
                                      }))
                                    );
