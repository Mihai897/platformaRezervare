...
const RezervareDetaliiSejur = ({hotel,room}) => {
    ...
    <div>
      ...
      <div className='mt-4'>
        ...
        <ul className='mt-4 space-y-3'>
          {room?.tarife.map((par,i)=>(

            <li key={i} onClick={()=>setSelectedId(i)} className={`flex max-modf8:flex-col max-modf8:space-y-3 modf8:justify-between border px-2 py-2 rounded-lg cursor-pointer border-button/30 ${selectedId ===i ? "bg-button/10 ": "bg-transparent "}`}>
            <div className=''>
              <div className='flex space-x-3'>
                <input onChange={()=>setSelectedId(i)} type='checkbox' checked={selectedId===i} />
                <p className='font-medium'>{par.titluTarif}</p>
              </div>
              <ul className='list-disc ml-10 marker:text-button text-[15px] text-gray-400'>
                {
                  par?.listaBen.map((par1,i1)=>(
                    <li key={i1}>{par1}</li>
                  ))
                }
              </ul>
            ...
            <div className='flex items-center'>
              <p className='text-gray-400'><span className='text-[24px] text-white font-medium'>{par.pretTarif}</span> RON / <span className='text-[16px]'>noapte</span></p>
            </div>
          </li>
          ))}
        </ul>
