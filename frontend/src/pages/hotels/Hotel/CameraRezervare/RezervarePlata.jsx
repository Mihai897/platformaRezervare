...
const RezervarePlata = ({hotel,room, setSearchParams}) => {
  ...
  return (
    <div>
      ...
      <ul className='space-y-3 mt-3'>
        ...
        {room?.metodePlata.map((par,i)=>
        { const Icon = iconMap[par.icon]
        return (
          <li key={i} onClick={()=>setSelectId(i)} className={`border px-3 py-1.5 rounded-lg border-button/30 hover:bg-button/10 transition-all duration-300 ease-in-out  cursor-pointer ${selectId===i? "bg-button/10": "bg-transparent"}`}>
            <div className='flex space-x-3'>
              <input onChange={()=>setSelectId(i)} type="checkbox" checked={selectId === i}/>
              <div className='text-[14px] flex items-center space-x-3'>
                {Icon && <Icon className="text-button" size={19}/>}
                <div>
                  <p className='font-medium'>{par.numePlata}</p>
                  <p className='text-gray-400'>{par.textPlata}</p>
                </div>
              </div>
            </div>

            <div className={`border-t hover:cursor-auto border-t-button/30 mt-3 py-3 ${selectId===i  && i===1? "flex flex-col":'hidden'}`}>
