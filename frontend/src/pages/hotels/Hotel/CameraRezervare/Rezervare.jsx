...
const Rezervare = () => {
  const {hotel,room} = useOutletContext();
  ...
  const showDiv = location.pathname === `/hotels/${hotel.slug}/camere/${room.slugs}/rezervare/rezervare-confirmata`
  return (
    <div className='max-w-screen-2xl mx-auto px-15 max-modf:px-10 max-modf1:px-8  '>
      ...
      <div className='flex items-center space-x-2 max-modf1:space-x-1 overflow-x-scroll scrollbar-hide text-[14px] text-gray-400'>
        ...
        <Link className='shrink-0' to={`/hotels/${hotel.slug}`}>{hotel.nume}</Link>
        <MdKeyboardArrowRight className='shrink-0'/>
        <Link className='shrink-0' to={`/hotels/${hotel.slug}/camere`}>Camere</Link>
        <MdKeyboardArrowRight className='shrink-0'/>
        <Link className='shrink-0' to={`/hotels/${hotel.slug}/camere/${room.slugs}`}>{room.title}</Link>
        <MdKeyboardArrowRight className='shrink-0'/>
      ...
      <div className='flex mt-4 max-modf:flex-col modf:space-x-4 items-start'>
          ...
          {!showDiv &&(<div className='border px-3 py-3 border-button/30 rounded-lg  mt-3'>
            {
              isDetSejur && (
              <RezervareDetaliiSejur hotel={hotel} room={room}/>)
            }
            {
              isDetOaspeti && (
              <RezervareDetaliiOaspeti hotel={hotel} room={room} setSearchParams={setSearchParams}/>)
            }
            {
              isDetPlata && (
              <RezervarePlata hotel={hotel} room={room} setSearchParams={setSearchParams}/>)
            }
        ...
        <div className='modf:flex-1 max-modf:w-full border px-3 py-3 max-modf:mt-3 border-button/30 rounded-lg text-[14px]'>
          ...
          <div className='flex space-x-3 items-center mt-3 border-b pb-3 border-b-button/30'>
            <div className='rounded-sm w-33 h-25 shrink-0 max-modf:w-30'>
              <img className='rounded-sm w-full h-full shrink-0 object-center' src={room.image} alt="" />
            </div>
            <div>
              <p>{room.title}</p>
              <p className='text-gray-400'>{hotel.nume}</p>
          ...
          <div className='py-3 border-b border-b-button/30'>
            <div className='text-gray-400 flex justify-between'>
              <p>{room.price} RON x 3 nopti</p>
              <p>{3*room.price} RON</p>
            </div>
            <div className='text-gray-400 flex justify-between'>
