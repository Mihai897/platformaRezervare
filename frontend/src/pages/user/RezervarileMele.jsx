...
const RezervarileMele = () => {
  ...
  const nrTotatalRezervari =  client[0].rezervare.length
  const nrTotalConfirmate = client[0].rezervare.filter(rezervare=>rezervare.status==="Confirmata").length
  const nrTotalDesfasurare = client[0].rezervare.filter(rezervare=>rezervare.status==="In desfasurare").length
  const nrTotalFinalizate = client[0].rezervare.filter(rezervare=>rezervare.status==="Finalizata").length
  const nrTotalAnulate = client[0].rezervare.filter(rezervare=>rezervare.status==="Anulata").length
  const statusFiltru = [
      ...
      {!codRezervare &&<div>
        ...
        <div className='flex modf6:space-x-3 max-modf6:flex-col max-modf7:space-y-3 mt-3 items-start'>
          ...
          <div className='border max-modf6:w-full border-button/20 px-3 py-3 rounded-lg w-80 shrink-0 shadow-lg bg-button1'>
            ...
            <div className='mt-3'>
              <button className='px-3 py-1.5 rounded-lg border border-button/30 hover:bg-button/30 transition-all duration-300 ease-in-out flex items-center justify-center space-x-3 cursor-pointer w-full'>
          ...
          <div className='border border-button/20 px-3 min-w-0 py-3 rounded-lg bg-button1 shadow-lg w-full'>
            <ul className='overflow-x-auto  overflow-hidden scrollbar-thin space-y-3 min-w-0 pb-4'>
              {
                client[0].rezervare.slice(0,visibleMoreRezervari).map((rezerv,iRezerv)=>(
                  <li className='border-y border-y-button/80 px-3 py-3 flex space-x-3 text-[14px] items-start min-w-max text-gray-400'>
                    ...
                    <div className='shrink-0 modf9:flex-2 max-modf9:w-79 transition-[width] duration-300 ease-in-out'>
                      <div className='flex space-x-2'>
                        <div className='w-25 h-20 rounded-lg shrink-0'>
                          <img src={rezerv.cameraImg} className='w-full h-full shrink-0 rounded-lg object-center' alt="" />
                        </div>
                        <div>
                          <p className='text-white'>{rezerv.numeHotel}, <span className='text-gray-400'>{rezerv.numeCamera}</span></p>
                          <p>{rezerv.locatiRez}</p>
                          <div className='flex items-center space-x-1.5'>
                            <ul className='flex space-x-1'>
                              {
                                [1,2,3,4,5].map((star,iStar)=>
                                <li key={iStar}>
                                  <FaStar className={`${star<=Math.round(rezerv.recenzieCam)?"text-yellow-400":"text-gray-400"}`}/>
                                </li>)
                              }
                            </ul>
                            <p className='text-white bg-button/60 rounded-sm px-1.5'>{rezerv.recenzieCam}</p>
                          </div>
                        </div>
                      </div>

                      <p className='mt-3'>Cod rezervare: {rezerv.codRezervare}, rezervat pe {rezerv.rezervareFacuta}</p>
                    </div>


                    <div className='shrink-0  modf9:flex-1  max-modf9:w-35 transition-[width] duration-300 ease-in-out'>                
                      <p>Check-in</p>
                      <p className='text-white mt-2'>{rezerv.checkIn }</p>
                    ...
                    <div className='shrink-0  modf9:flex-1 max-modf9:w-35 transition-[width] duration-300 ease-in-out'>
                      ...
                      <p>Check-out</p>
                      <p className='text-white mt-2'>{rezerv.checkOut }</p>
                    ...
                    <div className='shrink-0  modf9:flex-1 max-modf9:w-21 transition-[width] duration-300 ease-in-out'>
                      ...
                      <p>Oaspeti</p>
                      <p className='text-white mt-2'>{rezerv.nrOaspetiAdulti }</p>
                      <p>{rezerv.nrOaspetiCopii}</p>
                    </div>
                    ...
                    <div className='shrink-0 modf9:flex-1 flex flex-col items-start  max-modf9:w-26 transition-[width] duration-300 ease-in-out'>
                      <p className={`px-1.5 py-0.5 rounded-sm ${
                        rezerv.status==="Confirmata"?"text-button bg-button/30":
                        rezerv.status==="Anulata"?"text-red-600 bg-red-600/30":
                        rezerv.status==="Finalizata"?"bg-green-400/30 text-green-400":"text-blue-400 bg-blue-400/30"}`}>
                          {rezerv.status}</p>
                      <p className='mt-2.5'>Total platit</p>
                      <p className='text-white text-[16px]'>{rezerv.pretTotalNr} RON</p>
                    </div>


                    <div className='shrink-0  modf9:flex-1   space-x-3  my-auto max-modf9:w-30 transition-[width] duration-300 ease-in-out'>
                      <button onClick={()=>navigate(`/rezervarile-mele/${rezerv.codRezervare}`)} className='border border-button/30 px-1.5 py-1 rounded-lg cursor-pointer text-white hover:bg-button/30 transition-all duration-300 ease-in-out w-full'>Vezi detalii</button>
