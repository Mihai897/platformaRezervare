...
const DetaliiRezervariPage = () => {
  ...
  const clientul = client.find(c=> c.codClient === "Cl1")
  const rezervare = clientul.rezervare.find(r=>r.codRezervare === codRezervare)
    ...
    <div>
      {
        showDet && (
          <div>
                    ...
                    <div className='border border-button max-h-[95vh] overflow-hidden overflow-y-auto bg-button1 px-3 py-3 w-120 scrollbar-thin rounded-lg'>
                      ...
                      <div className='border border-button/30 px-3 py-3 mt-3 rounded-sm flex space-x-3'>
                        <div className='w-25 h-20 shrink-0'>
                          <img className='w-full rounded-lg h-full object-center shrink-0' src={rezervare.cameraImg} alt="" />
                        </div>
                        <div>
                          <p>{rezervare.numeHotel}, <span className='text-gray-400'>{rezervare.numeCamera}</span></p>
                          <p className='text-gray-400'>{rezervare.checkIn} - {rezervare.checkOut}</p>
                          <p className='text-gray-400'>{rezervare.nrOaspetiAdulti}, {rezervare.nrOaspetiCopii}</p>
                        </div>
                      </div>
            ...
            {
              (rezervare.status ==="In desfasurare" || rezervare.status === "Confirmata") && (
            ...
            <div className='mt-3 relative'>
              <img className='w-full max-modf2:h-69 h-115 max-modf:h-80 shrink-0 object-center' src={rezervare.cameraImg} alt="" />
              <p className={`absolute top-3 py-1 left-3 ${rezervare.status === "Confirmata"?" bg-button":rezervare.status==="Anulata"?" bg-red-600": rezervare.status==="Finalizata"?"bg-green-600 ":"bg-blue-400"} px-3 rounded-sm`}>{rezervare.status}</p>
              <div className={`absolute top-3 py-3 right-3 $ px-3 flex items-center space-x-1.5 bg-green-600 rounded-lg`}><span>{rezervare.recenzieCam} </span> <FaStar className='text-yellow-400'/></div>
            </div>

            <div className='mt-3'>
              <p className='text-[19px]'>{rezervare.numeHotel}, <span className='text-gray-400'>{rezervare.numeCamera}</span></p>
              <div className='flex space-x-3 items-center text-gray-400 mt-1'>
                <IoLocationOutline/>
                <p>{rezervare.locatiRez}</p>
              </div>
                ...
                <ul className='flex space-x-1'>
                  {
                    [1,2,3,4,5].map((star,istar)=>(
                      <li className={`${star<=Math.round(rezervare.recenzieCam)?"text-yellow-400":"text-gray-400"}`} key={istar}>
                        <FaStar/>
                      </li>
                    ))
                  }
                </ul>
                <p>{rezervare.recenzieCam} (128 recenzii)</p>
              </div>
              ...
              <div className='flex items-center modf8:space-x-4 mt-5 max-modf8:items-start max-modf8:space-y-3 max-modf8:flex-col text-gray-400'>
                <p className={`${rezervare.status === "Confirmata"?"text-button bg-button/30":rezervare.status==="Anulata"?"text-red-600 bg-red-600/30": rezervare.status==="Finalizata"?"bg-green-400/30 text-green-400":"bg-blue-400/30 text-blue-400"} shrink-0 px-3 rounded-sm`}>{rezervare.status}</p>
                <p>Cod rezervare: {rezervare.codRezervare}, Rezervat pe {rezervare.rezervareFacuta}</p>
              </div>
            </div>
            ...
            <div className='grid max-modf:grid-cols-2 grid-cols-4 max-modf8:grid-cols-1 gap-3 mt-3'>
              <div className='border px-3 py-3 rounded-lg border-button/30 flex items-start space-x-3'>
                ...
                <div>
                  ...
                  <p>{rezervare.checkIn}</p>
                  <p className='text-gray-400'>{rezervare.oraCheckIn}</p>
                </div>
              </div>
              ...
              <div className='border px-3 py-3 rounded-lg border-button/30 flex items-start space-x-3'>
                ...
                <div>
                  ...
                  <p>{rezervare.checkOut}</p>
                  <p className='text-gray-400'>{rezervare.oraCheckOut}</p>
                </div>
              </div>
              ...
              <div className='border px-3 py-3 rounded-lg border-button/30 flex items-start space-x-3'>
                ...
                <div>
                  ...
                  <p>{rezervare.nrOaspetiAdulti}</p>
                  <p className='text-gray-400'>{rezervare.nrOaspetiCopii}</p>
                </div>
              </div>
             ...
             <div className='border px-3 py-3 rounded-lg border-button/30'>
              ...
              <div className='mt-3 space-y-3'>
                <div className='flex space-x-3 justify-between'>
                  <p className='text-gray-400'>{rezervare.numeCamera} ({rezervare.periadaCazare})</p>
                  <p>{rezervare.pretTotal}</p>
                </div>
                <div className='flex space-x-3 justify-between'>
                ...
                <div className='border-t mt-5 py-3 flex space-x-3 justify-between items-center border-button/30'>
                   ...
                   <p className='text-button text-[20px]'>{rezervare.pretTotalNr +30} RON</p>
                </div>
              </div>
             </div>
            </div>
