...
const Navbar = ({visibleConnect,setVisibleConnect}) => {
  ...
  return (
    <div className='flex justify-between items-center py-4 max-modf1:relative z-50 max-w-screen-2xl mx-auto px-15 max-modf:px-10 max-modf1:px-8 max-modf:text-[14px]  border-b border-b-button/30'>
      ...
      <div className={`flex  space-x-6 max-modf:space-x-2 max-modf1:absolute max-modf1:flex-col max-modf1:top-14 max-modf1:bg-background max-modf1:right-0 max-modf1:left-0 max-modf1:px-8 max-modf1:transition-all max-modf1:duration-300  max-modf1:ease-in-out max-modf1:overflow-hidden max-modf1:pb-1 ${visible ? "max-modf1:max-h-50 max-modf1:opacity-100" : "max-modf1:max-h-0 max-modf1:opacity-0"}`}>
        {
          navbar.map((g)=>(
      ...
      <div className='space-x-4 flex max-modf:space-x-3 items-center'>
...

        {
          visibleConnect && <div className='relative mr-8'>
            ...
            <div  className={`absolute w-140 max-modf:w-125 max-modf1:w-100 max-modf8:w-80  bg-background border-button border  ${visibleMeniuNotificari?"max-h-100 opacity-100":"max-h-0 opacity-0 overflow-hidden"}  rounded-lg left-1/2 max-modf8:-left-19 top-9 modf8:-translate-x-1/2  z-20`}>
              <div className='relative '>
                ...
                <ul className={`max-h-85 overflow-hidden overflow-y-scroll space-y-3 py-3 scrollbar-none`}>
...

                  {
                    ...
                    client[0].notificari.slice(0,3).map((notificare,iNotificare)=>{
                      
                      const IconNot = iconMapNotificari[notificare.icon]
                      
                      return(
                      <li key={iNotificare} className='flex space-x-3 items-start text-[15px] px-3 border-y border-y-button/30 hover:bg-button/30 cursor-pointer transition-all duration-300 ease-in-out py-3'>
                        ...
                        <div className={`my-auto w-3 h-3 ${notificare.check===false?"bg-button":""} rounded-full`}></div>
                      </li>
                    )})
                ...
                <div className='absolute w-3 h-3 bg-button  -top-3 modf8:left-1/2 max-modf8:left-20 modf8:-translate-x-1/2' 
                style={{clipPath:"polygon(50% 0%,100% 100%,0% 100%)"}}></div>
                <div className='absolute w-3 h-3 bg-background   -top-2.75 modf8:left-1/2 max-modf8:left-20 modf8:-translate-x-1/2' 
                style={{clipPath:"polygon(50% 0%,100% 100%,0% 100%)"}}></div>
              </div>
            </div>
